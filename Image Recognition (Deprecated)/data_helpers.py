...
def load_data():
    ...
    ys = []
    for i in range(1, 6):
        filename = 'cifar-10-batches-py/data_batch_'+str(1)
...
def gen_batch(data, batch_size, num_iter):
    ...
    for i in range(num_iter):
        ...
        if (index + batch_size > len(data)):
            index = 0
            shuffled_indiced = np.random.permutation(np.arange(len(data)))
            data = data[shuffled_indiced]
...
def main():
    ...
    print(data_sets['images_text'].shape)
